(function($) {
		...
		input_change_value = function() {
			...
			if(!is_locked()&&picker_ctrl) {
				...
				var
					...
					// .replace(/\b(n)\b/g, m)
					// .replace(/\b(j)\b/g, d);
					str = d+"-"+i18n[pickers[picker.id].lang].months.short[m-1]+"-"+date.getFullYear();
	...
	.on('click',function(e) {
		if(picker) {
			if(!picker.input.is(e.target) && !picker.element.is(e.target) && picker.element.has(e.target).length === 0 && e.target.classList[0]=="overlay") {
				picker_hide();
				pick_dragged = null;
			}
		}
